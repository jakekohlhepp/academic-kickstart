---
title: Caregiving and Labor Supply: New Evidence from Administrative Data
slug: maestas-messel-truskinovsky-2024-caregiving-and-labor-supply
authors: Nicole Maestas|Matthew Messel|Yulya Truskinovsky
year: 2024
journal: Journal of Labor Economics
volume: 42
issue: S1
pages: S183-S218
doi: 10.1086/728810
source_url: https://www.nber.org/papers/w31450
pdf_path: library/pdfs/maestas-messel-truskinovsky-2024-caregiving-and-labor-supply.pdf
copy_type: NBER working-paper version of the peer-reviewed article
tags: caregiving|administrative data|family constraints|extensive margin|recent
related_themes: care-institutions-and-labor-supply|life-cycle-and-family-labor-supply|recent-directions-in-labor-supply-research|current-gaps-in-labor-supply-literature
thread_title: Care institutions define the feasible work set
thread_note: Child care, school schedules, leave, and elder care alter participation and continuity by changing whether market work is feasible, not merely whether it is attractive at the margin.
thread_articles: baker-gruber-milligan-2008-universal-child-care|cascio-2009-public-preschool-maternal-labor-supply|han-ruhm-waldfogel-2009-parental-leave-policies|blau-kahn-2007-married-women-labor-supply|attanasio-levell-low-sanchez-marcos-2015-aggregating-elasticities
shelf: recent
deck: A recent administrative-data paper showing that caregiving obligations materially reduce labor supply, making clear how incomplete care systems shape work decisions.
---
## Why it matters

This paper is a strong reminder that labor supply is embedded in care systems. When caregiving needs rise, labor supply can fall sharply even without any change in wages or taxes.

## Short summary

Maestas, Messel, and Truskinovsky use administrative data to estimate how caregiving affects labor supply. They find substantial reductions in work linked to caregiving responsibilities, with important persistence over time. The paper shows that family care demands create real labor-supply constraints rather than small transitory inconveniences.

For this wiki, the paper belongs with institution-focused work because it highlights what happens when care has to be privately absorbed. The weaker the surrounding support system, the more caregiving becomes a labor-supply shock.

## What to carry forward

- Care responsibilities can create large and persistent labor-supply effects.
- Family constraints are central, not secondary, for many workers.
- Administrative data are especially valuable for tracing dynamic caregiving effects.

## Caution

This paper identifies the effect of caregiving burdens more directly than it identifies which exact institutional reform would offset them. It is therefore strongest as a constraint-mapping paper.

## Easy-to-copy BibTeX

```bibtex
@article{MaestasMesselTruskinovsky2024CaregivingLaborSupply,
  author = {Maestas, Nicole and Messel, Matthew and Truskinovsky, Yulya},
  title = {Caregiving and Labor Supply: New Evidence from Administrative Data},
  journal = {Journal of Labor Economics},
  year = {2024},
  volume = {42},
  number = {S1},
  pages = {S183--S218},
  doi = {10.1086/728810}
}
```
